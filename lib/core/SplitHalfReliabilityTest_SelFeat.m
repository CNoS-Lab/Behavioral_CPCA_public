function predictStruct = SplitHalfReliabilityTest_SelFeat(G_CPCA, nIter, load_cut_off, pred_cut_off, Sel_feat)

Z = G_CPCA.Z;
G = G_CPCA.G;
n = G_CPCA.n;

Rej_feat = setdiff(1:size(Z,2), Sel_feat);

disp(Sel_feat);

if ~isempty(Rej_feat)
    G_CPCA_temp = G_CPCA_Analysis(Z(:,Sel_feat), Z(:,Rej_feat), [], false);
    Z = G_CPCA_temp.E;
end

G_CPCA.n = min(n, rank(Z));

G_CPCA.loadings_VD_sqrtN_GH = G_CPCA.loadings_VD_sqrtN_GH(Sel_feat,1:min(n,rank(Z)));
