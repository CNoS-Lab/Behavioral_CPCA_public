function predictStruct = SplitHalfReliabilityTest_SelFeat_no_regress(G_CPCA, nIter, load_cut_off, pred_cut_off, Sel_feat)
...
G_CPCA.n = min(n, rank(Z));

G_CPCA.loadings_VD_sqrtN_GH = G_CPCA.loadings_VD_sqrtN_GH(:,1:min(n,rank(Z)));
...
parfor ii = 1:nIter
    ...
    G_CPCA_CV_1 = G_CPCA_Analysis(Z(cv_idx(:,ii)~=1,:), G(cv_idx(:,ii)~=1,:), min(n,rank(Z)), false);
    G_CPCA_CV_2 = G_CPCA_Analysis(Z(cv_idx(:,ii)~=2,:), G(cv_idx(:,ii)~=2,:), min(n,rank(Z)), false);
    
    matchStruct(ii) = matchComp(G_CPCA, [G_CPCA_CV_1,G_CPCA_CV_2], load_cut_off, 'orthogonal', Sel_feat);    
end
...
function outStruct = matchComp(G_CPCA, G_CPCA_CV, load_cut_off, type, Sel_feat)
...
Rej_feat = setdiff(1:size(G_CPCA.Z,2),Sel_feat);
...
V = G_CPCA_CV(1).VGH(:,1:n);
D = G_CPCA_CV(1).DGH(1:n,1:n);
weights = V/D*Ta*sqrt(N-1);
weights(Rej_feat,:) = 0;

G_CPCA_CV(1).scores_NU_GH = G_CPCA_CV(1).S*weights;
G_CPCA_CV(1).PCorr = corr(G_CPCA_CV(1).scores_NU_GH, G_CPCA_CV(1).G)';

V = G_CPCA_CV(2).VGH(:,1:n);
D = G_CPCA_CV(2).DGH(1:n,1:n);
weights = V/D*Tb*sqrt(N-1);
weights(Rej_feat,:) = 0;

G_CPCA_CV(2).scores_NU_GH = G_CPCA_CV(2).S*weights;
